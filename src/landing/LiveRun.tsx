...
const GRAPH: Graph = {
  ...
  nodes: [
    ...
    },
    {
      ...
      label: "billing reply",
      config: {
        prompt: "Write a billing reply to: {{ ticket }}",
        outputKey: "reply",
      },
    },
    {
      ...
      config: {
        prompt: "Write a troubleshooting reply to: {{ ticket }}",
        outputKey: "reply",
      },
    },
    {
      ...
      type: "approval",
      label: "human gate",
      config: { message: "Send to a person?" },
    },
...
const llmFor = (intent: string) =>
  new FakeLLM([
    ...
    [
      "billing reply",
      "Sorry about the double charge. The duplicate is refunded.",
    ],
...
export function LiveRun() {
  ...
  const traversed = new Set(
    steps.slice(1).map((s, i) => `${steps[i].nodeId}->${s.nodeId}`),
  );
  const cost = steps.reduce((sum, s) => sum + (s.meta?.costUsd ?? 0), 0);

  const changed = current
    ? Object.keys(current.patch).filter(
        (k) => show(current.stateBefore[k]) !== show(current.stateAfter[k]),
      )
    : [];
              ...
              return (
                ...
                >
                  ...
                  {(() => {
                    ...
                    // A lucide icon is an <svg>, and a nested <svg> takes x/y, so it drops
                    // straight into the graph's coordinate space.
                    return (
                      <Icon
                        x={p.x + 9}
                        y={p.y + H / 2 - 6}
        ...
        <div className="flex flex-col bg-panel">
          ...
          <div className="tnum mt-auto flex items-center gap-4 border-t border-line px-4 py-2.5 font-mono text-micro text-text-faint">
            ...
            <span className="ml-auto text-text-dim">
              {TICKETS[pass % TICKETS.length].intent} path
            </span>
          </div>
        </div>
      </div>
