...
export const useStore = create<Store>((set, get) => {
  ...
  const finish = async (result: RunResult) => {
    gen = null;
    set({
      status: result.status === "done" ? "done" : (result.status as RunStatus),
  ...
  const pump = async (once: boolean) => {
    ...
    while (gen === active) {
      ...
      set((s) => ({
        steps: [...s.steps, { ...event, step: s.steps.length }],
        currentStep: s.steps.length,
      }));

      if (event.status === "awaiting-approval")
        return set({ status: "awaiting-approval" });
      if (event.status === "error") continue; // the generator returns on the next pull
      if (once) return set({ status: "paused" });
      if (event.next && get().breakpoints.includes(event.next))
  ...
  return {
    ...
    toggleBreakpoint: (id) =>
      set((s) => ({
        breakpoints: s.breakpoints.includes(id)
          ? s.breakpoints.filter((b) => b !== id)
          : [...s.breakpoints, id],
      })),
    ...
    editState: (patch) =>
      set({ pendingEdit: { ...(get().pendingEdit ?? {}), ...patch } }),
