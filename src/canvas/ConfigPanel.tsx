...
import { TypeIcon } from "./LoomNodeView.tsx";
  ...
  return (
    <div className="space-y-5 p-5">
      <div className="flex items-center gap-2.5">
        <TypeIcon type={data.type} size={16} />
        <SectionTitle>{data.type}</SectionTitle>
        <span className="ml-auto font-mono text-micro text-text-faint">
          {selectedId}
        </span>
      </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Writes to">
              <input
                className={mono}
                value={cfg.outputKey ?? ""}
                onChange={(e) => setCfg({ outputKey: e.target.value })}
              />
            </Field>
      ...
      {data.type === "condition" && (
        <Field label="Branches" hint="first match wins, otherwise else">
          <div className="space-y-2">
            {((cfg.branches ?? []) as { handle: string; expr: string }[]).map(
              (b, i) => (
                <div key={i} className="flex gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      const branches = [
                        ...(cfg.branches as { handle: string; expr: string }[]),
                      ];
                  ...
                  <button
                    ...
                    onClick={() =>
                      setCfg({
                        branches: (cfg.branches as unknown[]).filter(
                          (_, j) => j !== i,
                        ),
                      })
                    }
            ...
            <button
              className="inline-flex items-center gap-1 text-meta text-pick transition-opacity hover:opacity-80"
              onClick={() =>
                setCfg({
                  branches: [
                    ...((cfg.branches ?? []) as unknown[]),
                    { handle: "new", expr: "true" },
                  ],
                })
              }
            >
        ...
        <>
          <Field label="For each" hint="state key holding an array">
            <input
              className={mono}
              value={cfg.over ?? ""}
              onChange={(e) => setCfg({ over: e.target.value })}
            />
          </Field>
          <Field label="Or repeat while" hint="expression">
            <input
              className={mono}
              value={cfg.while ?? ""}
              onChange={(e) => setCfg({ while: e.target.value })}
            />
          </Field>
          <div className="grid grid-cols-2 gap-3">
            <Field label="Each item lands in">
              <input
                className={mono}
                value={cfg.itemKey ?? ""}
                onChange={(e) => setCfg({ itemKey: e.target.value })}
              />
            </Field>
            <Field label="Stop after">
              <input
                ...
                onChange={(e) =>
                  setCfg({ maxIterations: Number(e.target.value) })
                }
              />
            </Field>
          </div>
      ...
      <div className="space-y-2.5 rounded-lg border border-line bg-ink-900 p-3">
        ...
        <div className="flex gap-3">
          <span className="w-12 shrink-0 text-meta text-text-faint">
            Writes
          </span>
          <span className="font-mono text-meta text-live">
            {writesOf(data).join("  ") || "nothing"}
          </span>
        </div>
      </div>
      ...
      <button
        ...
        className="inline-flex items-center gap-1.5 text-meta text-text-faint transition-colors hover:text-halt"
      >
