...
export function TypeDot({ type, size = 7 }: { type: string; size?: number }) {
  return (
    <span
      aria-hidden
...
function summarize(data: LoomNode['data']): string {
  ...
  switch (data.type) {
    ...
    case 'tool':
      return c.kind === 'js' ? `worker js → ${c.outputKey}` : `${c.method ?? 'GET'} → ${c.outputKey}`
    case 'condition': {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5 border-b border-line px-3 py-2.5">
        ...
        <span className="min-w-0 flex-1 truncate text-ui font-medium text-text">{data.label}</span>
        <button
          aria-label={armed ? `Clear breakpoint on ${data.label}` : `Break before ${data.label}`}
          aria-pressed={armed}
          ...
          className={[
            ...
            armed ? 'border-halt bg-halt' : 'border-line-bright hover:border-halt',
          ].join(' ')}
        />
      </div>

      <div className="space-y-1.5 px-3 py-2.5">
        <div className="truncate font-mono text-meta text-text-dim">{summarize(data)}</div>
        <div className="tnum flex gap-3 font-mono text-micro text-text-faint">
          {!stat ? (
            <span>Not run yet</span>
          ) : stat.avgLatencyMs === undefined && stat.avgCostUsd === undefined ? (
          ...
          {stat && stat.errors > 0 && <span className="text-halt">{stat.errors} failed</span>}
        </div>
      </div>
      ...
      {branches.length === 0 ? (
        ...
        <div className="border-t border-line py-1">
          {branches.map((handle) => (
            <div key={handle} className="relative px-3 py-1 text-right font-mono text-micro text-text-dim">
              {handle}
