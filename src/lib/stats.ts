...
export function nodeStats(runs: RunRecord[]): Record<string, NodeStat> {
  ...
  for (const run of runs) {
    for (const step of run.steps) {
      const s = (out[step.nodeId] ??= { visits: 0, errors: 0, totalCostUsd: 0, _lat: [], _cost: [] })
      s.visits++
...
export function pathCounts(runs: RunRecord[]): { source: string; target: string; value: number }[] {
...
export const usd = (n?: number) => (n === undefined ? '-' : n < 0.01 ? `$${n.toFixed(4)}` : `$${n.toFixed(2)}`)
export const ms = (n?: number) =>
  n === undefined ? '-' : n < 1000 ? `${Math.round(n)}ms` : `${(n / 1000).toFixed(1)}s`
