...
import type { AgentNode, Graph, NodeType } from '../../packages/engine/index.ts'
...
export const NODE_TYPES: NodeType[] = ['input', 'llm', 'tool', 'condition', 'loop', 'approval', 'output']

export const DEFAULT_CONFIG: Record<NodeType, Record<string, unknown>> = {
  input: { seed: {} },
  llm: { prompt: 'Answer this: {{ question }}', outputKey: 'answer', model: 'claude-opus-5' },
  tool: { kind: 'http', url: 'https://api.example.com/{{ id }}', method: 'GET', outputKey: 'result' },
...
export function readsOf(data: NodeData): string[] {
  ...
  for (const text of sources) {
    for (const [, path] of text.matchAll(/\{\{\s*([\w.$]+)\s*\}\}/g)) keys.add(path)
  }
  ...
  for (const b of (c.branches as { expr: string }[]) ?? []) {
    for (const [, id] of b.expr.matchAll(/\b([a-zA-Z_$][\w$]*)\b/g)) {
      if (!['true', 'false', 'null', 'undefined', 'typeof', 'includes', 'length'].includes(id)) keys.add(id)
    }
  }
  return [...keys]
}
...
export function writesOf(data: NodeData): string[] {
  ...
  if (data.type === 'input') keys.push(...Object.keys((c.seed as object) ?? {}))
  return keys
}

/** Canvas -> engine. The one place editor shapes get cast into engine shapes. */
export function toGraph(nodes: LoomNode[], edges: RFEdge[], meta: Partial<Graph> = {}): Graph {
  const entry = nodes.find((n) => n.data.type === 'input')?.id ?? nodes[0]?.id
  if (!entry) throw new Error('The graph needs at least one node')
  return {
    ...meta,
    entry,
    nodes: nodes.map((n) => ({ id: n.id, type: n.data.type, label: n.data.label, config: n.data.config }) as AgentNode),
