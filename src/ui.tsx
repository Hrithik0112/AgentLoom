  ...
  return (
    <label className="block">
      <span className="mb-1.5 flex items-baseline gap-2">
        ...
        {hint && <span className="font-mono text-micro text-text-faint">{hint}</span>}
      </span>
      {children}
    </label>
  )
 ...
 */
export function useResizablePanel(storageKey: string, initial: number, min: number, max: number) {
  ...
  useEffect(() => {
    const fit = () => setWidth((w) => clamp(w, min, Math.min(max, window.innerWidth - 360)))
