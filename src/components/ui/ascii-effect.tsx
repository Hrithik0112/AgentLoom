  ...
  useEffect(() => {
    ...
    const draw = (now: number) => {
      ...
      const drawWidth =
        fit === "stretch"
          ? columns
          : (image.naturalWidth * imageScale) / cellWidth;
      const drawHeight =
        fit === "stretch"
          ? rows
          : (image.naturalHeight * imageScale) / cellHeight;
      ...
      for (let row = 0; row < rows; row++) {
        ...
        for (let column = 0; column < columns; column++) {
          ...
          if (variant === "flow" && !reduceMotion) {
            ...
            const phase =
              (x * directionX + y * directionY) * flowFrequency +
              now * flowSpeed * Math.PI * 0.002;
            const crossPhase =
              (-x * directionY + y * directionX) * flowFrequency * 0.65;
            const drift =
              (Math.sin(phase) + Math.sin(phase * 0.61 + crossPhase) * 0.45) *
              flowStrength;
            ...
            if (pointer.current.active && mouseRadius > 0) {
              ...
              if (distance > 0 && mouseInfluence > 0) {
                const ripple = Math.sin(
                  distance * 0.055 - now * mouseWaveSpeed * Math.PI * 0.002,
                );
                mouseDisplacement =
                  mouseInfluence ** 2 * mouseStrength * ripple;
                sourceColumn -=
                  ((mouseX / distance) * mouseDisplacement) / cellWidth;
                sourceRow -=
                  ((mouseY / distance) * mouseDisplacement) / cellHeight;
              }
            }
          ...
          let luminance = clamp(
            luminanceField[sampleIndex]! + mouseInfluence * 0.08,
          );
...
export function AsciiImage(props: AsciiPresetProps) {
  return (
    <AsciiEffect
      {...props}
      variant="image"
      colors={props.colors ?? IMAGE_COLORS}
    />
  );
}
...
export function AsciiFlow(props: AsciiPresetProps) {
  return (
    <AsciiEffect
      {...props}
      variant="flow"
      colors={props.colors ?? FLOW_COLORS}
    />
  );
}
...
export function AsciiGlitch(props: AsciiPresetProps) {
  return (
    <AsciiEffect
      {...props}
      variant="glitch"
      colors={props.colors ?? GLITCH_COLORS}
    />
  );
}
