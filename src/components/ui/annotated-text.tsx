    ...
    const animations = Array.from(drawings, (drawing, index) => {
      ...
      const animation = drawing.animate(
            ...
            ],
        {
          ...
          // Patched from "backwards". Backwards fill holds the first keyframe, which is
          // opacity 0, so the mark stayed invisible before the draw and reverted to the
          // undashed resting state after it. "both" holds the drawn state at each end.
          fill: "both",
        },
      );
      animation.pause();
